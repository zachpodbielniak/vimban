...
_resolve_remote = vimban._resolve_remote
_remote_request = vimban._remote_request
...
# ============================================================================
# REMOTE TICKET LOADING
# ============================================================================
# module-level remote state — set at startup if --remote is active
_tui_remote_url: str | None = None
_tui_remote_token: str | None = None

# Thread-safe queue for collab SSE events fed by the background daemon thread
_collab_queue: '_queue_module.Queue[dict]' = _queue_module.Queue()


def _collab_sse_thread(remote_url: str, token: str) -> None:
    """Background daemon thread: subscribe to SSE and feed events into _collab_queue."""
                ...
                for raw in resp:
                    ...
                    if line.startswith('data:'):
                        ...
                        try:
                            ...
                            _collab_queue.put_nowait(evt)
                        except Exception:
                            pass
                        buffer = ''
        except Exception:
            time.sleep(5)  # reconnect after 5s on any error


def _handle_collab_event(evt: dict, tui: Any) -> None:
    """Handle an SSE event in the TUI main loop (called from VimbanTUI.run)."""
    t = evt.get('type', '')
    d = evt.get('data', {})

    if t in ('TICKET_CREATED', 'TICKET_UPDATED', 'TICKET_ARCHIVED'):
        tui.state_manager.refresh_data()
        tui._last_refresh_time = time.monotonic()
    elif t in ('PRESENCE_JOIN', 'PRESENCE_LEAVE', 'CONNECTED'):
        presence = d.get('presence', [])
        tui.collab_presence = presence
        # Trigger a redraw so the header shows updated presence
        tui.state_manager.refresh_data()
        tui._last_refresh_time = time.monotonic()


def _remote_list_tickets(
    directory: Path,
    ...
    """
    Load tickets from a remote vimban_serve API.

    Drop-in replacement for fallback_list_tickets() that fetches data
    from the remote API instead of the local filesystem. Returns Ticket
    objects by constructing them from JSON data.

    Args:
        directory: Ignored in remote mode (kept for API compat)
        filters: Dict of filter keys (status, type, assignee, etc.)
    ...
    try:
        resp: dict = _remote_request(_tui_remote_url, "GET", "/api/tickets", _tui_remote_token, params=params)
    except SystemExit:
        return []

    tickets: list = []
    raw_list: list = resp.get("tickets", [])
    for item in raw_list:
        # skip excluded types
        if exclude_types and item.get("type", "") in exclude_types:
            continue
        # construct a Ticket object from the JSON dict
        # the API returns simplified fields (strings) that need to be
        # converted to the types expected by the Ticket dataclass
        try:
            # parse created datetime (required field, must have a value)
            created_val: datetime = datetime.now()
            if item.get("created"):
                try:
                    created_val = datetime.fromisoformat(str(item["created"]))
                except (ValueError, TypeError):
                    pass

            # parse updated datetime (optional)
            updated_val: datetime | None = None
            if item.get("updated"):
                try:
                    updated_val = datetime.fromisoformat(str(item["updated"]))
                except (ValueError, TypeError):
                    pass

            # parse filepath (required field, use dummy if not available)
            filepath_val: Path = Path(item["filepath"]) if item.get("filepath") else Path("/tmp/remote-ticket")

            # parse people fields: API returns plain strings like "zach_podbielniak"
            # but Ticket expects TransclusionLink objects
            assignee_str: str = item.get("assignee", "") or ""
            assignee_val: TransclusionLink | None = None
            if assignee_str:
                # try parsing as transclusion first, then create a simple link
                assignee_val = TransclusionLink.parse(assignee_str)
                if assignee_val is None:
                    assignee_val = TransclusionLink(path=assignee_str)

            reporter_str: str = item.get("reporter", "") or ""
            reporter_val: TransclusionLink | None = None
            if reporter_str:
                reporter_val = TransclusionLink.parse(reporter_str)
                if reporter_val is None:
                    reporter_val = TransclusionLink(path=reporter_str)

            # parse tags: API returns comma-separated string or list
            raw_tags: Any = item.get("tags", [])
            tags_val: list[str] = []
            ...
            if item.get("due_date"):
                try:
                    due_date_val = date.fromisoformat(str(item["due_date"]))
                except (ValueError, TypeError):
                    pass
            ...
            if item.get("end_date"):
                try:
                    end_date_val = date.fromisoformat(str(item["end_date"]))
                except (ValueError, TypeError):
                    pass
    ...
    def _render_token(self, token: dict, lines: list, current_line: list,
        ...
        if token_type == 'heading':
            ...
            for i, item in enumerate(token.get('children', [])):
                prefix: str = f"{i+1}. " if ordered else "• "
...
class VimbanTUI:
    ...
    def __init__(
        ...
        self.remote_token: str | None = remote_token

        # Presence list updated by _handle_collab_event from the SSE background thread
        self.collab_presence: list = []
    ...
    def _vimban_base_cmd(self) -> list[str]:
        """
        Build the base vimban command with remote flags if active.

        Returns a list like ['vimban', '--directory', '...'] or
        ['vimban', '--remote', 'http://...', '--api-token', '...']
        when remote mode is enabled.

        Returns:
            Base command list to extend with subcommand args
        """
        cmd: list[str] = ['vimban']
        if self.remote_url:
            cmd.extend(['--remote', self.remote_url])
            if self.remote_token:
                cmd.extend(['--api-token', self.remote_token])
            else:
                cmd.append('--no-token')
        else:
            cmd.extend(['--directory', str(self.vimban_config.directory)])
        return cmd
    ...
    def run(self) -> int:
        ...
        while True:
            ...
            # Check for collab events from background SSE thread
            while not _collab_queue.empty():
                try:
                    evt = _collab_queue.get_nowait()
                    _handle_collab_event(evt, self)
    ...
    def _draw_header(self, width: int) -> None:
        ...
        # Show connected collaborators when in remote mode
        if self.remote_url and self.collab_presence:
            users: str = ",".join(str(u) for u in self.collab_presence[:5])
            right = f"[collab:{users}] {right}"
...
def main(stdscr: Any, args: argparse.Namespace) -> int:
    ...
    # Resolve remote settings
    remote_url: str | None = None
    remote_token: str | None = None
    if getattr(args, 'remote', None):
        remote_url, remote_token, _tui_project_keys = _resolve_remote(args.remote)
        if getattr(args, 'api_token', None):
            remote_token = args.api_token
        if getattr(args, 'no_token', False):
            remote_token = None

        # set module-level remote state so _remote_list_tickets works
        global _tui_remote_url, _tui_remote_token, fallback_list_tickets
        _tui_remote_url = remote_url
        _tui_remote_token = remote_token
        # swap out the data loader for remote mode
        fallback_list_tickets = _remote_list_tickets

    # Start background SSE collab thread when running in remote mode
    if remote_url:
        t = threading.Thread(
            target=_collab_sse_thread,
            args=(remote_url, remote_token or ''),
            daemon=True,
        )
        t.start()

    # Create and run TUI
    tui: VimbanTUI = VimbanTUI(stdscr, vimban_config, tui_config, remote_url, remote_token)
    return tui.run()
...
if __name__ == "__main__":
    ...
    # Startup pull unless disabled (skip in remote mode — no local git)
    if not args.no_pull and not getattr(args, 'remote', None):
