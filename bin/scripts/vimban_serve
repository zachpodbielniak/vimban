...
async def api_projects():
    """Return all unique project keys with open ticket counts."""
    if _proxy_enabled:
        resp: dict = _remote_request(_proxy_url, "GET", "/api/projects", _proxy_token)
        return jsonify(resp)
    tickets: list[dict[str, Any]] = list_tickets(include_done=False)
    ...
    projects: list[dict] = sorted(
        [{'name': k, 'count': v} for k, v in project_counts.items()],
        key=lambda x: x['name']
    )
...
def _proxy_vimban_cmd(args: list[str], timeout: int = 30) -> tuple[int, str, str]:
    ...
    try:
        if cmd == "list":
            ...
            resp = _remote_request(_proxy_url, "GET", "/api/projects", _proxy_token)
            return (0, json.dumps(resp.get("projects", [])), "")
